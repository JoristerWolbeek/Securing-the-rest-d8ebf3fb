...
<body>
            ...
            if (isset($_GET["movieTitel"])) {
                if ($_GET["movieTitel"] == "titel") {
                    ...
                    while ($info = $stmt->fetch()) {
                        echo ("<tr><td>" . $info['titel'] . "</td><td>" . $info['duur'] .
                        "</td><td><a name='id' type='submit' href='http://localhost/films.php?id=$info[id]'>Details</a></td></tr>");
                    }
                }
            }
    ...
    <form method=post>
            ...
            if (isset($_POST['makeFilm'])) {
                $createFilm = $pdo->query('INSERT INTO netland.inhoud 
    (titel, duur, taal, landVanAfkomst, omschrijving, trailer, soort) VALUES ("0", 0, "0", "0", "0", "0,", "movies")');
                $createFilm->fetch();
                header("refresh:0");
            }
            ?>
    ...
    <form action="series.php" method="get">
            ...
            if (isset($_GET["serieTitle"])) {
                if ($_GET["serieTitle"] == "title") {
                    ...
                    while ($info = $stmt1->fetch()) {
                        echo ("<tr><td>" . $info['title'] . "</td><td>" . $info['rating'] .
                         "</td><td><a name='id' type='submit' href='http://localhost/series.php?id=$info[id]'>Details</a></td></tr>");
                    }
                }
            }
            ...
            if ($stmt1 == "") {
                ...
                while ($info = $stmt1->fetch()) {
                    echo ("<tr><td>" . $info['titel'] . "</td><td>" . $info['rating'] .
                    "</td><td><a name='id' type='submit' href='http://localhost/series.php?id=$info[id]'>Details</a></td></tr>");
                }
            }
            ?>
    ...
    <form method=post>
            ...
            if (isset($_POST['makeSerie'])) {
                $createSerie = $pdo->query('INSERT INTO inhoud (titel, rating, omschrijving, 
    awards, seizoenen, landVanAfkomst, taal, soort) VALUES ("0", 0, "0", 0, 0, "0,", "0", "series")');
                $createSerie->fetch();
                header("refresh:0");
            }
            ?>
